...
bool ParseTextureSet(const uint8_t* data, size_t size, std::string& diffuse) {
    Cursor c(data, size);
    ...
    for (uint32_t i = 0; i < numTextures; ++i) {
        std::string value;
        if (!c.SizedString(value)) return false;
        if (i == 0) diffuse = value;
        FQ_LOGI("Q5H TEXTURE SLOT: index=%u path=%s", i,
                value.empty() ? "<empty>" : value.c_str());
    }
    return !diffuse.empty();
}

bool ParseTriStripsData(const uint8_t* data, size_t size, Fo3TexturedChairMesh& outMesh,
    ...
    if ((bsDataFlags & 0x0001u) != 0) {
        texcoords.reserve(static_cast<size_t>(numVertices) * 2u);
        for (uint16_t i = 0; i < numVertices; ++i) {
            float u = 0.0f, v = 0.0f;
        ...
        FQ_LOGE("Q5H chair NiTriStripsData has no UV set");
        return false;
    }
    ...
    if (indices.empty() || texcoords.size() != static_cast<size_t>(numVertices) * 2u) {
        return false;
    }
...
bool LoadMegatonChairTexturedMesh(Fo3TexturedChairMesh& outMesh) {
    ...
    if (!LoadChairNif(nif)) {
        FQ_LOGE("Q5H chair NIF extraction failed");
        return false;
    }

    NifHeader header;
    if (!ParseHeader(nif, header)) {
        FQ_LOGE("Q5H chair NIF header parse failed");
        return false;
    }
    ...
    for (uint32_t block = 0; block < header.numBlocks; ++block) {
        ...
        if (type == "BSShaderTextureSet") {
            std::string diffuse;
            if (ParseTextureSet(nif.data() + offset, blockSize, diffuse)) {
                outMesh.diffuseTexturePath = diffuse;
                haveDiffuse = true;
                FQ_LOGI("Q5H DIFFUSE: block=%u path=%s", block, diffuse.c_str());
            }
        } else if (type == "NiTriStripsData") {
            if (!ParseTriStripsData(nif.data() + offset, blockSize,
                                    outMesh, declaredTriangles)) {
                FQ_LOGE("Q5H failed to decode NiTriStripsData block=%u", block);
                return false;
            }
            haveGeometry = true;
        }
    ...
    if (!haveGeometry) {
        FQ_LOGE("Q5H no NiTriStripsData geometry found");
        return false;
    }
    if (!haveDiffuse) {
        FQ_LOGE("Q5H no diffuse texture path found in BSShaderTextureSet");
        return false;
    }

    FQ_LOGI("Q5H NIF READY: vertices=%zu triangles=%zu uvs=%zu diffuse=%s declaredTriangles=%u",
            outMesh.positions.size() / 3u,
            outMesh.indices.size() / 3u,
            outMesh.texcoords.size() / 2u,
            outMesh.diffuseTexturePath.c_str(), declaredTriangles);
    return true;
}
