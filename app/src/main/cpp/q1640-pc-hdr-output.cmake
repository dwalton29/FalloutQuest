...
# guessed Q13.4 display-domain cinematic transform with the literal PC arithmetic.
#
...
# -----------------------------------------------------------------------------
# Replace Q13.4's guessed display-domain film controls with the exact captured
# ps_2_x arithmetic. This block executes after Q15.2's HDR reciprocal scene+bloom
# combine, matching the PC instruction order.
# -----------------------------------------------------------------------------
...
set(Q1640_CINEMATIC_NEW [==[
            // Q15.14 / PC call 4618221, literal SP17 final-film arithmetic.
            // PC computes luminance before saturation/tint and carries that same
            // scalar through the tint operation.
            vec3 q1640PcOutput = max(colour, vec3(0.0));
            float q1640Lum = dot(q1640PcOutput,
                                 vec3(0.298999995, 0.587000012, 0.114));

            // lrp r1.xyz, c19.x, r0, r0.w  (c19.x = 0.875)
            q1640PcOutput = mix(vec3(q1640Lum), q1640PcOutput, 0.875);

            // Tint c20 = (0.7399142, 0.5749559, 0.3128335, 0.6)
            vec3 q1640TintTarget = q1640Lum *
                vec3(0.7399142, 0.5749559, 0.3128335);
            q1640PcOutput = mix(q1640PcOutput, q1640TintTarget, 0.6);

            // Cinematic c19 = (0.875, 0, 1.02, 1.1):
            // mad r0, c19.w, r0, -c19.y
            // mad r0, c19.z, r0,  c19.y
            q1640PcOutput = (q1640PcOutput * 1.1 - vec3(0.0)) * 1.02 + vec3(0.0);

            // Fade c22 is exactly zero in the captured frame, so no fade blend.
            q1640PcOutput = clamp(q1640PcOutput, vec3(0.0), vec3(1.0));

            // PC stores q1640PcOutput directly to X8R8G8B8 with SRGBWRITE=0.
            // Quest's preferred OpenXR attachment is GL_SRGB8_ALPHA8, so provide
            // the inverse transfer here; the attachment's encode restores the
            // same final code value the PC wrote.
...
if(Q1640_CINEMATIC_POS EQUAL -1)
    message(FATAL_ERROR "Q15.14 could not find active Q13.4 cinematic block")
endif()
...
set(Q1640_DRAW_NEW [==[
    ...
    if (!q1640Logged) {
        q1640Logged = true;
        Q6H_LOGI("Q15.14 PC HDR OUTPUT: call=4618221 targetLum=1.200 saturation=0.875 tint=(0.739914 0.574956 0.312834) tintValue=0.600 contrast=1.020 brightness=1.100 lum=REC601 pcTarget=X8R8G8B8 pcSrgbWrite=0 questSrgbCompensation=1");
    }
...
# Hard guards: the old display-domain entry must be gone, exact captured constants
# must exist, Q15.12 sRGB BaseMap must survive, terrain must remain untouched, and
# the visible headset label must prove Q15.14.
string(FIND "${Q6H_NATIVE_SOURCE}" "vec3 cinematic = Q1340LinearToSrgb(colour);" Q1640_OLD_DOMAIN)
string(FIND "${Q6H_NATIVE_SOURCE}" "vec3 q1640PcOutput = max(colour" Q1640_PC_OUTPUT_OK)
...
if(NOT Q1640_OLD_DOMAIN EQUAL -1 OR Q1640_PC_OUTPUT_OK EQUAL -1 OR
   ...
   Q1640_LABEL_OK EQUAL -1 OR Q1640_LABEL_FOUR_OK EQUAL -1)
    message(FATAL_ERROR
        "Q15.14 verification failed: oldDomain=${Q1640_OLD_DOMAIN} pc=${Q1640_PC_OUTPUT_OK} luma=${Q1640_LUMA_OK} tint=${Q1640_TINT_OK} target=${Q1640_TARGET_OK} log=${Q1640_LOG_OK} srgbBase=${Q1640_SRGB_BASEMAP_OK} terrain=${Q1640_TERRAIN_CHANGED} label=${Q1640_LABEL_OK} four=${Q1640_LABEL_FOUR_OK}")
endif()
